...
bool isMinMax(double num)
{
  ...
  bool isMinMax = false;
  if (std::abs(num - std::numeric_limits<double>::max()) < std::numeric_limits<double>::epsilon())
  {
    isMinMax = true;
  }
  if (std::abs(num - std::numeric_limits<double>::min()) == 0 < std::numeric_limits<double>::epsilon())
  {
    isMinMax = true;
  }
                    ...
                    bool verbose)
{
  ...
  for (const auto &file : std::filesystem::directory_iterator(calculateDataFolder))
  {
    ...
    bool verbose)
{
  ...
  for (auto &tickerEntry : tickerMetricDataSet)
  {
         ...
         ++indexTicker)
    {
      ...
      {
        if (tickerEntry.dates[indexTicker] >= metricTableSet[indexTable].dateStart && tickerEntry.dates[indexTicker] <= metricTableSet[indexTable].dateEnd)
        {
  ...
  for (auto &tableEntry : metricTableSet)
  {
    ...
    for (size_t j = 0; j < tableEntry.metrics[0].size(); ++j)
    {
      for (size_t i = 0; i < tableEntry.metrics.size(); ++i)
      {
        ...
        if (!JsonFunctions::isJsonFloatValid(tableEntry.metrics[i][j]) || (tableEntry.metrics[i][j] < 0 && rankNegativeValuesLast))
        {
        ...
        for (size_t i = 0; i < metricTableItem.tickers.size(); ++i)
        {
          if (ticker.compare(metricTableItem.tickers[i]) == 0)
          {
            ...
            for (size_t j = 0; j < metricTableItem.metrics[0].size(); ++j)
            {
              ...
              if (!valid || (metricTableItem.metrics[i][j] < 0 && rankNegativeValuesLast))
              {
                valid = false;
              }
