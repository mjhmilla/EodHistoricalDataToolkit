...
int main (int argc, char* argv[]) {
  ...
  std::string exchangeListJsonFileName;
  ...
  try{
    ...
    TCLAP::ValueArg<std::string> exchangeListJsonFileNameInput("l","exchange_list", 
      "The exchanges-list json file generated by EOD",
      false,"","string");

    cmd.add(exchangeListJsonFileNameInput);    
    ...
    exchangeListJsonFileName  = exchangeListJsonFileNameInput.getValue();
    ...
    if(tickerFileListPath.length()==0 
        ...
        && exchangeListJsonFileName.length()==0){
    ...
    if(exchangeCode.length() > 0 && exchangeListJsonFileName.length() > 0){
      throw std::invalid_argument(
        "Cannot set both the exchange code (-x) and exchange list "
        "file name (-l).");               
    }

    if(tickerFileListPath.length() >0 && exchangeListJsonFileName.length() >0){
      throw std::invalid_argument(
    ...
    if(exchangeListJsonFileName.length() > 0){
    ...
    if(verbose){
      ...
      std::cout << "    " << exchangeListJsonFileName << std::endl;
  ...
  if(mode==MODE_FETCH_MULTIPLE_EXCHANGE_FILES){
    std::ifstream exchangeListPathStream(exchangeListJsonFileName.c_str());
